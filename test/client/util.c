...
//******************************************************************************
//******************************************************************************
static void do_register(const bevt_client_id_t id) {
    int ret = bevt_client_register(id, BEVT_CLIENT_PRIO_DEFAULT);
    if(ret<0)
...
//******************************************************************************
//******************************************************************************
static void do_subscribe(const bevt_client_id_t id) {
    int ret = bevt_client_subscribe(id, BEVT_CLIENT_PRIO_DEFAULT);
    if(ret<0)
...
int main(int argc, char const *const *argv) {
...

    {
        ...
        for (;;)
        {
            register int opt = subgetopt_r(argc, argv, "r:u:s:v:p", &l) ;
            if (opt == -1) break ;
            switch (opt)
            {
                case 'r' : action=0; id=atoi(l.arg); break ;
                case 'u' : action=1; id=atoi(l.arg); break ;
                case 's' : action=2; id=atoi(l.arg); break ;
                case 'v' : action=3; id=atoi(l.arg); break ;
                case 'p' : action=4; break ;
                default : strerr_dieusage(100, USAGE) ;
            }

            switch(action) {
            case 0: do_register(id); break;
            case 1: do_unregister(id); break;
            case 2: do_subscribe(id); break;
